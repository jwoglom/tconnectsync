...
def format_datetime(date):
	# Convert to UTC and format as ISO-8601 with Z suffix (Nightscout expects UTC)
	return arrow.get(date).to('UTC').replace(microsecond=0).isoformat().replace('+00:00', 'Z')
...
class NightscoutApi:
	...
	def last_uploaded_entry(self, eventType, time_start=None, time_end=None):
		dateFilter = time_range('created_at', time_start, time_end, t_to_space=False)
		...
		if latest.status_code != 200:
			if 'as a valid ISO-8601 date' in latest.text:
				logger.warning("Nightscout last_uploaded_entry %s could not process ISO-8601 date: start=%s end=%s dateFilter=%s response=%s" % (eventType, time_start, time_end, dateFilter, latest.text))
				return None
	...
	def last_uploaded_bg_entry(self, time_start=None, time_end=None):
		dateFilter = time_range('dateString', time_start, time_end, t_to_space=False)
		...
		if latest.status_code != 200:
			if 'as a valid ISO-8601 date' in latest.text:
				logger.warning("Nightscout last_uploaded_bg_entry could not process ISO-8601 date: start=%s end=%s dateFilter=%s response=%s" % (time_start, time_end, dateFilter, latest.text))
				return None
	...
	def last_uploaded_activity(self, activityType, time_start=None, time_end=None):
		...
		if latest.status_code != 200:
			if 'as a valid ISO-8601 date' in latest.text:
				logger.warning("Nightscout activity %s could not process ISO-8601 date: start=%s end=%s dateFilter=%s response=%s" % (activityType, time_start, time_end, dateFilter, latest.text))
				return None
	...
	def last_uploaded_devicestatus(self, time_start=None, time_end=None):
		...
		if latest.status_code != 200:
			if 'as a valid ISO-8601 date' in latest.text:
				logger.warning("Nightscout devicestatus could not process ISO-8601 date: start=%s end=%s dateFilter=%s response=%s" % (time_start, time_end, dateFilter, latest.text))
				return None
